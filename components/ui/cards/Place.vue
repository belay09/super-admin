...
/**-----------------------Close Place for The day Modal---------------- */
const showCloseThePlaceForTheDayModal = ref(false);
function toggleCloseThePlaceForTheDayModal(event) {
  event.stopPropagation();
  showCloseThePlaceForTheDayModal.value =
    !showCloseThePlaceForTheDayModal.value;
}
...
<template>
  ...
  <Modals-Modal
    :autoClose="true"
    :modelValue="showCloseThePlaceForTheDayModal"
    bodyClass="lg:max-w-2xl"
  >
    <template #header>
      <div class="flex items-center justify-between pb-4">
        ...
        <button @click="showCloseThePlaceForTheDayModal = false">
          <Icon name="system-uicons:close" class="text-4xl" />
        </button>
      </div>
    </template>
    <template #content>
      <PlacesClosePlace
        @close="showCloseThePlaceForTheDayModal = false"
        :place="place"
      ></PlacesClosePlace>
    </template>
  </Modals-Modal>
  ...
  <div
    ...
    class="flex flex-col hover:border-primary-600 border hover:shadow-md max-w-lg p-[25px] rounded-xl text-sm hover:cursor-pointer"
  >
    <!-- Header -->
