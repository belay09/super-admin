...
const placeItemRefetch = inject("placeItemRefetch");
const placeReviewRefetch = inject("placeReviewRefetch");
const { notify } = useNotify();
/**--------------------------Report category data fetch---------------- */

const filter = ref({
  type: {
    _eq: props.place?.type,
  },
});
    ...
    <div class="flex justify-between my-8 w-full">
      <div class="flex flex-col space-y-4">
        <p class="whitespace-nowrap xl:text-xl font-medium dark:text-white">
          Rate your experience with {{ place?.name }}
        </p>
      </div>
