  ...
  <div class="w-full">
    ...
    <div class="relative rounded-md shadow-sm w-full">
      <input
        v-model="inputValue"
        :disabled="disabled"
        :style="`background:  url('data:image/svg+xml,${svgIcon}') right no-repeat;`"
        type="date"
        @input="input($event)"
        @change="input($event)"
        :id="id"
        ...
        :class="[
          ...
          props.disabled
            ? ' bg-white border-gray-200 hover:border-gray-200 text-gray-400 pointer-events-none shadow-none'
            : ' focus:ring-1',
          props.class,
        ]"
        class="block w-full text-primary-text focus:outline-none text-base rounded-md"
        aria-invalid="true"
