...
const { mutate, onError, onDone, loading } = uploadMutation(
  upload_query,
  enabled
);
const upload = () => {
...
onDone(({ data }) => {
  if (data) {
    let temp = [...uploadedFiles.value, ...data.upload_files.data.fileUrls];
...
<template>
  <HModal v-model="openModalVal" :autoClose="true">
    <template #Heading>
    ...
    <template #content>
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <div v-if="file.type != 'application/pdf'" class="w-20 h-20">
                <img
                  role="presentation"
                  :alt="file.name"
                  :src="file.objectURL"
      ...
      <div class="flex justify-end w-full mt-5 gap-x-5">
        <button
        ...
        <button
          @click="upload"
          class="flex gap-x-2 disabled:cursor-not-allowed disabled:bg-gray-300 disabled:border-none disabled:text-gray-500 btn-primary"
