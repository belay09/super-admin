...
<template>
  <div class="w-full relative">
    ...
    >
      ...
      >
        ...
        <Icon
          ...
          class="ml-1 w-4 h-4 hover:cursor-pointer hover:text-primary-600"
          @click="deleteChip(i)"
          >clear</Icon
        >
      </div>
    ...
    <ul
      ref="list_select"
      v-show="show"
      class="absolute z-10 w-full bg-white border border-primary-600 shadow max-h-56 rounded-b-md text-base overflow-auto"
      :class="[listClass || '']"
    >
      <li class="h-1">
        <h-progress
          ...
          color1="bg-primary-600-lite"
          color2="bg-primary-600"
          color3="bg-primary-600-dark"
          height="h-1"
        ></h-progress>
      </li>
      <li class="px-2 border-b pb-2">
        <input
          class="flex-1 focus:ring-0 focus:outline-none flex min-w-20 rounded-md border-primary-600 border-2 w-full p-1"
          name="select"
          @click="show = true"
          v-model="search"
          @input="queryList"
      ...
      <li
        v-for="item in items"
        @click="select(item)"
        :class="[multiple && selected[item[value]] ? 'bg-primary-600/20' : '']"
        class="border-b select-none relative py-3 px-3 hover:bg-primary-600/30 text-gray-600 cursor-pointer"
      >
