...
onResult((result) => {
  if (result.data?.placeAds) {
    ...
    // Assuming placeAds is the array extracted from your data response

    // Step 1 & 2: Extract endDate values and convert them to Date objects
    const endDates = placeAds.value.map((ad) => new Date(ad.endDate));

    // Step 3: Find the latest date
// Outputs: 2024-06-29
    const date = new Date(Math.max(...endDates));
    latestDate.value = date.toISOString().split('T')[0];

    // latestDate now holds the greatest date, which you can use for validation
    console.log("hello",latestDate.value); // Outputs the date in YYYY-MM-DD format
  }
});
...
<template>
  ...
  <!-- -----------------------Add ad space modal----------------------- -->
  <AdSpace-PlaceAdAdd :place-type="adType" v-model="openAddAdSpace" :last-date="latestDate"/>
