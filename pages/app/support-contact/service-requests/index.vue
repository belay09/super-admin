<script setup>
import getBusinessReq from "~/graphql/query/business-request/list.gql";
import editBusinessRequest from "@/graphql/mutations/business-request/edit.gql";
...
onResult(({ data }) => {
	serviceReq.value = data.businessRequests;
...
const selectedBusinessReqId = ref();
...
} = authMutation(editBusinessRequest);

const onSubmit = () => {
	markAsAddressed({
		id: selectedBusinessReqId.value,
		input: {
